...
const LamborghiniCardDetails = () => {
     const [Lamborghini, setLamborghini] = useState([]);

     const { id } = useParams();
   
     useEffect(() => {
       fetch("https://carbazar-server-site.vercel.app/Lamborghini")
         .then((res) => res.json())
         .then((data) => {
             setLamborghini(data)
         });
     ...
     return (
       <div className="m-32 bg-slate-200 rounded-lg">
         ...
         <div className="mt-20 text-center">
           ...
           <p className="p-20">{find?.Short_description} {find?.Short_description} {find?.Short_description}</p>
         </div>
       </div>
     );
};

export default LamborghiniCardDetails;